...
class Oauth2UserCredentials implements GrantTypeInterface
{
    ...
    public function validateRequest(RequestInterface $request, ResponseInterface $response)
    {
		...
		$password = $this->rsaDecrypt(base64_decode($request->request("password")), $privateKey);

        if (!password_verify($password, $userInfo['password'])) {
            $response->setError(401, 'invalid_credentials', 'Invalid username and password combination');
